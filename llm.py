import random
from wikipedia_digest import convert_url, wikipedia_digest, clean_list


def get_file_decision() -> str:
    """Prompts the user for which type of text they want generated."""
    file = input(
        """What Wikipedia would you like to emulate?:  (1-3)
1. Kanye West
2. Canadian History
3. Donald Trump
>>> """
...
>>> """
    ...
    if int(file) == 1:
        return "https://en.wikipedia.org/wiki/Kanye_West"
    elif int(file) == 2:
        return "https://en.wikipedia.org/wiki/History_of_Canada"
    else:
        return "https://en.wikipedia.org/wiki/Donald_Trump"
...
def get_numwords_and_clength() -> tuple[int, int]:
    ...
    while not clength.isalnum() or int(clength) <= 0 or int(clength) > num_words:
        print(
            'Invalid. Must be a positive number smaller then \
                number of words'
        )
...
def context_dictionary(words_list: list[str], context_length: int) -> dict[tuple, list[str]]:
...
def generate_text(context_dict: dict[tuple[str], list[str]], num_words: int) -> str:
...
def generate_random_output() -> str:
    """Return randomly generated output with num_words words based on a context
    of context_length words from the training list of words word_list.
    """

    num_words, context_length = get_numwords_and_clength()

    url = get_file_decision()
    text = convert_url(url)
    raw_list = wikipedia_digest(text)
    word_list = clean_list(raw_list)
